...
def convert_text_to_qrcode(text, filename, request):
    ...
    QRimg1.save(f"{settings.MEDIA_ROOT}/cvm_qrcodes/{filename}.jpg")
    domain = get_current_site(request).domain

    qr_code_image_path = f"/cvm_qrcodes/{filename}.jpg"
    qr_code_url = f"{domain}/media/cvm_qrcodes/{filename}.jpg"

    return qr_code_image_path, qr_code_url


def send_pass_code(email, code):
    subject = 'Your Pass Code For Kabadi Techno App'
